...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, path.join(__dirname, 'public', 'archivos'))
    },
...
const CRMCotizacionSchema = new mongoose.Schema({
    ...
    estado: { 
        type: String, 
        enum: ['Levantamiento', 'Cotizando', 'En Seguimiento', 'Aprobado', 'Perdido', 'Perdida', 'Terminada', 'Cerrada', 'En Proceso', 'Ganada'], 
        default: 'Ganada' 
    },
    partidas: [{
        descripcion: String,
...
app.post('/api/cotizaciones', async (req, res) => {
    try {
        const data = req.body;
        // Generar un folio si viene vacío
        if (!data.folio || data.folio.trim() === '') {
            const count = await CRMCotizacion.countDocuments();
            data.folio = `C-${String(count + 1).padStart(3, '0')}`;
        }
...
app.post('/api/proyectos', async (req, res) => {
    try {
        ...
        if (data.cotizacionId) {
            ...
            // Marcar cotizacion a "Cerrada" / "Aprobada" si se pasa a proyectos operativos
            if (cot) {
                 cot.proyectoActivoId = folioFinal; 
                 // No cambiamos el estado, lo mantenemos como Cerrada o Aprobado según decida el usuario, pero lo vinculamos.
                 await cot.save();
            }
        }
...
// Cambiar estado de actividad (empleado: En Camino / En Sitio; admin: Completada)
app.put('/api/actividades/:id/estado', async (req, res) => {
    try {
        const { id } = req.params;
        const { estado } = req.body;
        const allowed = ['Pendiente', 'En Camino', 'En Sitio', 'En Progreso', 'Completada'];
        if (!allowed.includes(estado)) return res.status(400).json({ error: 'Estado no válido' });
        const act = await CRMActividad.findByIdAndUpdate(id, { estado }, { new: true });
        if (!act) return res.status(404).json({ error: 'Actividad no encontrada' });
        res.json({ success: true, estado: act.estado });
    } catch(err) { res.status(500).json({ error: err.message }); }
});
